...
for i = 1, 9 do
	...
	keymap("n", ("<Leader>%s"):format(i), ("<Plug>(cokeline-switch-%s)"):format(i), opts)
end
...
keymap(
	"n",
	"<leader>x",
	"<cmd>let buf=bufnr('%') <bar> exec 'bufdo update' <bar> exec 'b' buf <bar> TermExec cmd='make run'<cr>",
	opts
)
keymap(
	"n",
	"<leader>xp",
	"<cmd>let buf=bufnr('%') <bar> exec 'bufdo update' <bar> exec 'b' buf <bar> TermExec cmd='python main.py'<cr>",
	opts
)
